...
# 출처를 「맨 아래 섹션」이 아니라 본문 곳곳에 다는 글이 있다 — 주장 옆 인용구,
# 문단 끝 한 줄. 그런 원문은 parse_refs 가 None 을 돌려주므로 예전에는 「원문에 출처가
# 없다」로 읽고 **조용히 통과**했다(2026-08-12 실측: 출처 3건이 빠졌는데 exit=0).
#
# @AI:FRAGILE 콜론까지 함께 본다. 「출처」 한 단어만 세면 "출처를 밝히지 않은" 같은
#   평범한 문장이 걸려 오탐이 난다. 실제 표기는 예외 없이 콜론이 따라온다
#   (`출처: …` / `출처 : [텍스트](URL)`). 전각 콜론도 받는다.
INLINE_SRC_RE = re.compile(r"출처\s*[:：]")
...
def check(orig_text, nav_text, internal_re=None):
    ...
    if not orig:
        # 섹션은 없는데 본문에 출처 표기가 흩어져 있으면, 이 검사기는 그것을 셀 수 없다.
        # 「못 센다」를 「없다」로 흘려보내면 아무것도 안 보면서 초록불을 주는 것이라 여기서 끊는다.
        inline = INLINE_SRC_RE.findall(orig_text)
        if inline:
            return False, [
                f"[REFS-NOT-SECTIONED] 원문에 「참고 자료」 섹션이 없는데 본문에 「출처:」 표기가 {len(inline)}건 있습니다.",
                "  이 형식은 이 검사기가 세지 못합니다 — 통과시키면 출처가 통째로 빠져도 모릅니다.",
                "  둘 중 하나로 진행하세요:",
                "    ① 원문 하단에 `## 참고 자료` 로 모은 뒤 다시 돌린다 (권장 — 그래야 다음 편부터 자동으로 잡힌다)",
                "    ② 네이버판에 옮긴 출처를 사람이 원문과 직접 대조한다",
            ]
...
CASES = [
    ...
    ("원문 줄에 한 마디만 붙인 복사", _ORIG,
     ...
     "- 전국 동물병원 진료비 현황조사 결과 — 농림축산식품부 자료실\n\n#가 #나\n", False),
    # 섹션 없이 본문에 흩어 놓은 출처 — 실측 형식 그대로(2026-08-12).
    # 이 케이스가 없으면 「출처 8건이 빠져도 exit=0」이 되어 검사기가 무력해진다.
    ("섹션 없이 본문에 흩어진 출처 — 조용히 통과하면 안 된다",
     "## 재혼 준비\n다시 시작하는 자리는 처음과 다릅니다.\n\n출처: 스페셜 재혼 (https://example.com/a)\n\n"
     "## 검증 절차\n서류 확인이 먼저입니다.\n\n출처 : 안심책임제 (https://example.com/b)\n",
     "## 본문\n다시 시작하는 자리는 다릅니다.\n\n#가 #나\n", False),
    # 위 판정의 오탐 방지 — 「출처」가 콜론 없이 평범한 문장에 나오는 경우는 잡지 않는다.
    ("「출처」가 콜론 없이 문장에 등장하면 잡지 않는다",
     "## 본문\n출처를 밝히지 않은 글은 신뢰를 잃습니다. 출처 없는 주장은 쓰지 않습니다.\n",
     "## 본문\n근거 없는 말은 쓰지 않습니다.\n\n#가 #나\n", True),
]
