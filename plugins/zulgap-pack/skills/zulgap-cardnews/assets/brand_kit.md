...
| 페이드 | **아웃만** — 개별 out 0.4s / 합본 out 0.6s (페이드인 금지, 함정 9) |
...
9. **[필수] 페이드인을 넣지 말 것 — 첫 프레임이 검정이면 SNS 커버가 죽는다.**
   인스타그램·페이스북은 **첫 프레임을 게시물 커버로** 쓴다. `fade=t=in` 이 있으면 그 프레임이
   완전한 검정이라 **피드에서 게시글이 까맣게 보인다** (탐색기 썸네일도 전부 검정).
   2026-08-19 재혼사이트 편에서 사장님이 발견. 그 전 편들(크리스찬·유학파)도 같은 상태로 나갔다.
   - 페이드**아웃**은 끝맺음이라 남긴다. 커버에 영향을 주지 않는다.
   - 고칠 때 **Seedance 재생성은 필요 없다.** 원본을 다시 내려받아 합성만 다시 하면 되고
     추가 과금이 없다.
   - 확인은 눈이 아니라 수치로 한다 — 첫 프레임 YAVG 가 0 근처면 검정이다:
     ```bash
     ffmpeg -i <mp4> -frames:v 1 -vf signalstats,metadata=print:key=lavfi.signalstats.YAVG -f null -
     ```
     재혼사이트 편은 고친 뒤 6장 전부 50~140 이 나왔다(1번은 밤 장면이라 50).
