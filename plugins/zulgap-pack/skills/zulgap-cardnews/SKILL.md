...
| 2 | **인스타그램 본문** | Step 4 |
| 3 | **해시태그 20개** | Step 4 |
| 4 | **노션 콘텐츠마스터 행 2개** — 인스타그램/페이스북 쌍 | Step 6 |

기획안(Step1~3)은 중간 산출물이다. **기획안만 내고 멈추지 말 것.**
영상까지 만들었으면 **Step 6(노션 기록)까지 이어서 한다** — 여기서 멈추면 매번 사람이 손으로 행을 만든다.
...
## 6. 🗂️ 노션 콘텐츠마스터 기록 (Step 6)

영상 납품 후 **항상 이어서** 한다. 사용자가 따로 시키지 않아도 진행한다.
카드뉴스 1건당 노션에 **행 2개** — 인스타그램 / 페이스북 **쌍으로**.
"페이스북에도 낼지"를 **묻지 말 것** — 같은 소재를 두 채널에 내는 게 기본 운영이다
(2026-08-04 지시. 과거 건 25년 5월/12월, 26년 7월도 전부 쌍이다).

### 6-1. 접근 경로 ← 여기서 한 번 막힌다
- DB: **콘텐츠마스터 관리 메인 DB** — `collection://7f410191-5034-4dae-8648-7e109bfffe27`
- **노션 커넥터(`notion-fetch`/`notion-create-pages`/`notion-update-page`)로 접근한다.**
  제디 `ext_notion_*` 는 이 DB에 권한이 없다 — integration `nAPI_forJDB` 에 공유되지 않아
  `Could not find block with ID` 로 실패한다.
- 확인용 뷰: 「카드뉴스」 (필터 = `콘텐츠 구분 ∈ {페이스북 카드뉴스, 인스타그램 카드뉴스}` + `월`)
  → **`월` 을 해당 월로 넣어야 이 뷰에 뜬다.**

### 6-2. 채우는 속성 (두 행 공통, `콘텐츠 구분`만 다름)

| 속성 | 값 |
|---|---|
| `기획안(👇클릭해주세요)` | 원본 블로그 제목 (두 행 동일) |
| `동료사명` | 엔노블 |
| `콘텐츠 구분` | 한 행은 `인스타그램 카드뉴스`, 다른 행은 `페이스북 카드뉴스` |
| `월` / `연` | 예: `26년 8월` / `2026년` — **뷰 필터가 `월` 이라 필수** |
| `단계` | 6-3 참고 |
| `동료사 마스터` | 담당자 |
| `예상 발행일` / `실 발행일` | 같은 값으로 넣는다 |
| `참고링크` | **원본 블로그의 노션 카드 URL** (콘텐츠마스터에서 제목으로 찾는다) |
| `특이사항` | 규격 + 미해결 사항 한 줄 |

- 날짜는 `date:<속성>:start` + `date:<속성>:is_datetime`.
  **`is_datetime` 은 숫자 0/1 이다** — 문자열 `"0"` 을 주면 400 validation_error.
- `발행 링크` 는 **비워 둔다.** 업로드해야 나오는 값이라 채울 수 없다 — 본문 체크리스트에 남긴다.
- 본문은 기존 카드처럼 빈 기사 템플릿을 복제하지 말고, **산출물 목록·검증 결과·남은 일**을 적는다.

### 6-3. 「단계」 — 임의로 발행완료를 박지 말 것

**업로드 여부를 한 번 묻고 그에 따라 넣는다.**

| 상황 | 단계 |
|---|---|
| 영상만 완성, 업로드 전 | `5. 초안작성완료` |
| 업로드까지 끝났다고 확인받음 | `11. 발행완료` + `실 발행일` = 실제 업로드일 |

> ⚠️ **7 이상은 원래 사람만 올리는 값이다.** 제디 `ext_notion_update_page` 는 발행 트리거라며
> 차단한다. 노션 커넥터엔 그 가드가 없으니 **명시적 확인을 받았을 때만** 올리고,
> 우회했다는 사실을 사용자에게 알린다.
> 확인 없이 `11` 을 박으면 **발행 링크가 빈 채로 "발행완료"인 카드**가 남아 월말 집계가 어긋난다.
...
- `월` 을 넣었는가? 안 넣으면 「카드뉴스」 뷰에 안 뜬다
- `참고링크` 에 원본 블로그 노션 카드를 걸었는가?
- **`단계` 를 확인 없이 7 이상으로 올리지 않았는가?** 업로드 여부를 물어보고 정할 것
