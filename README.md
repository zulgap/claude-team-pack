# 줄갭 팀원용 Claude Code 패키지 (claude-team-pack)

줄갭 팀원이 각자 윈도우 PC에서 **사장님처럼 콘텐츠 작업**을 할 수 있게 해주는 Claude 플러그인입니다.
직원은 **Claude 데스크탑 앱**으로 사용합니다.

## 포함된 것
- **스킬**: `시작`(노션 허브 현황 보기), `세션저널`(작업 기록)
- **MCP 도구**: 노션(자료 불러오기, 각자 로그인) · PPT(pptx) · 한글(hwp, ※ 한컴오피스 필요) · **제디(jedi, 회사 데이터 조회 — 개인 토큰 필요)**

---

## 🧑‍💻 직원 설치 (데스크탑 앱)

1. **https://claude.ai/download** 에서 Claude 데스크탑 앱 설치
2. **`install.bat` 더블클릭** → Git·Node·uv 자동 설치 (PPT·한글 도구용)
3. 앱 실행 → **같은 계정으로 로그인**
4. 앱 **설정/＋ 메뉴 → Plugins → Add marketplace** → GitHub 저장소 `zulgap/claude-team-pack` 입력
5. 목록에서 **zulgap 플러그인 → Install** → **Reload**(또는 앱 재시작)
6. 채팅창에 `/시작` 입력 → 노션 허브가 뜨면 **완료** 🎉

> 노션이 안 열리면 = 노션 워크스페이스 초대 필요 (노션은 직원이 각자 본인 계정으로 로그인).
> 메뉴 위치는 앱 버전마다 조금 다를 수 있어요.

### 🤖 제디(회사 데이터) 연결 — 선택
회사 데이터(판단·도구)를 제디로 조회하려면 **개인 토큰(JEDI_TOKEN)**이 필요합니다.
1. 사장님께 토큰 요청 → 사장님이 `railway run node scripts/issue-mcp-token.js --actor <당신ID> --tenant <테넌트ID>`로 발급해 한 줄을 줍니다.
2. `install.bat` 실행 중 `JEDI_TOKEN` 물어볼 때 붙여넣기 (이미 설치했으면: PowerShell에 `setx JEDI_TOKEN "<토큰>"` → 앱 재시작).
3. 앱 재시작 후 제디 도구가 활성화됩니다.

> 토큰은 **본인 권한·테넌트 안에서만** 동작하고 **읽기 위주**입니다. 토큰을 남에게 주지 마세요. 토큰이 없어도 노션·PPT·한글은 그대로 됩니다.

### (참고) 터미널(CLI) 사용자라면
```
/plugin marketplace add zulgap/claude-team-pack
/plugin install zulgap@zulgap-team-pack
/reload-plugins
```

---

## 🛠 사장님 업데이트 (= 직원 PC에 반영)

1. 이 repo 파일 수정 → `git push`
2. 직원: 앱 **Plugins 메뉴에서 Update/Reload** (CLI: `/plugin marketplace update zulgap-team-pack`)

새 스킬을 추가하려면 `skills/<이름>/SKILL.md` 폴더를 더하고 push 하면 됩니다.

---

## ⚠️ 주의 (보안)
- **production 키(Railway·Supabase·Cloudflare)는 포함하지 않습니다** — 사장님 전용.
- 같은 Claude 계정을 3명이 쓰면 사용 한도를 공유하니, 자주 막히면 계정 분리를 검토하세요.
- 데스크탑 앱은 PATH를 완전히 못 읽는 경우가 있어, PPT·한글 도구가 안 되면 PC를 한번 재시작하거나 사장님께 알려주세요.

## 🔐 키 공유 — 지금은 불필요 (나중에)
지금 직원 도구(노션·PPT·한글)는 **공유할 비밀번호가 없습니다** (노션은 각자 로그인). 나중에 직원과 **유료 API 키**(예: 이미지·영상 생성)를 공유하게 되면 → `docs/1password-setup.md` 참고.
